...
def main(args):
    ...
    with open(os.path.abspath('./embedding.pkl'), 'rb') as f:#
    ...
    # test
    logging.info('Processing test from {}'.format(args.dest_dir))
    test = preprocessor.get_dataset(
        args.dest_dir, args.n_workers,
        {'n_positive': -1, 'n_negative': -1, 'shuffle': False}
    )
    test_pkl_path = os.path.join(os.path.abspath('.'), 'test.pkl')
